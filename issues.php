...
<body>
  ...
  <?php
    ...
    if(isset($_POST['raiseissue']) && ! empty ($_POST['raiseissue']))
    {
      ...
      $host=$_SESSION["user"];
      $query="Insert into `issue` (`host`,`category`,`heading`,`issue`,`attachments`,`status`,`upvotes`, `location`) values('$host','$category','$title','$issue','$files','0','0', '$location')";
        ...
        <div class="modal-body">
          <form class="form" method="post" action="issues.php" enctype="multipart/form-data">
            ...
            <div class="row">
              ...
              <input type="text" class="form-control" name="location" placeholder="Enter your issue's Location" required>
            </div>
              </div>
            </div>
            <div class="row">
  ...
  <div class="modal fade" id="discuss" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true" style="z-index: 1400;">
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        ...
        <?php } ?>
        <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#comment" data-siid="<?= $id ?>" style="display:inline;">Comment</button>
        ...
        <br>
        <?php
        $id = $selectedissue['isid'];
        $sql="Select * from comments where discuss='$id'";
        $sql = mysqli_query($conn, $sql);
        ?>
        <h5 class="modal-title">Comments (<?php echo mysqli_num_rows($sql) ?>)</h5>
        <div class="card bg-light p-3" style="width:98%;">
          <ul style="margin: 0; padding: 13px;">
            <?php
            while ($rs = mysqli_fetch_array($sql)) {
              $tt = "SELECT name from user where uid = '".$rs['id']."'";
              $tt = mysqli_query($conn, $tt);
              $tt = mysqli_fetch_array($tt);
              echo '<li>'.$rs['comment'].' ('.$tt['name'].')</li>';
            }
             ?>
          </ul>
        </div>
    ...
    <div class="modal-body">
      <form action="comment.php" method="post">
        <div class="row">
          <div class="col-sm-12">
            <div class="form-group">
          <input type="text" class="form-control" name="cid" value="<?php echo $selectedissue['isid']; ?>" hidden>
    ...
    <div class="issue-body row">
        ...
        <div class="issues col-md-7">
          ...
          <div class="row">
            <div class="card">
            <div class="card-body">
              <h5 class="card-title"><b><?= $row['heading'] ?></b></h5>
              <p class="card-text"><i><?= $row['issue'] ?></i></p>
              <p class="card-text"><small class="text-muted"><i><strong>Location - </strong><?= $row['location'] ?></i></small></p>
        ...
        <div class="sidenav2 col-md-2">
          ...
          <ul class="list-group">
            ...
            <li class="list-group-item">Anmol</li>
            <li class="list-group-item">Dhruv</li>
            <li class="list-group-item">Prastik</li>
